...
//Normalization of a Vector. We use this mostly for the direction vector of a ray
Points Normalize(Points a){
    double r = sqrt(a.x*a.x + a.y*a.y + a.z*a.z);
    return Points(a.x/r, a.y/r, a.z/r);
}

// At First I didn't want to use color as a class. But It was getting troublesome to maintain
class Color{
public:
    double r, g, b;
    Color(){};
...
extern vector<PointLight> pointLights; // Holds the pointlights
extern vector<SpotLight> spotLights; // Holds the spotlights
extern vector<Object*> objects; // Holds the objects (Sphere, Triangle, Floor, General)
...
public:
    PointLight pl;
    Points direction; // Spotlight has extra two features. Direction (the direction in which the light is emitted) 
    double angle; //and Angle (the angle of the spotlight)
    SpotLight(){};
    void draw(){
        glPushMatrix();
...
public:
	...
	{
		return -1.0; // We have keep this getTminimum function virtual so that we can override it in the derived classes and use it in intersect function
	}
...
public:
    ...
    Color getColor(Points &p) override{
        ...
        // It gives the checkerboard pattern
    }
...
public:
   ...
   General(double A, double B, double C, double D, double E, double F, double G, double H, double I, double J, Points referencePoint, double length, double width, double height){
    ...
    bool isWithinRange(Points point)
    {
        ...
        // These checks are used for clipping the object
        return true;
    }
